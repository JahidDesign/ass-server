...
router.get('/', async (req, res) => {
  try {
    const bookings = await getToursCollection().find().toArray();
    res.json(bookings);
  } catch {
...
router.get('/:id', async (req, res) => {
  try {
    const booking = await getToursCollection().findOne({ _id: new ObjectId(req.params.id) });
...
router.post('/', async (req, res) => {
  try {
    const result = await getToursCollection().insertOne(req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await getToursCollection().deleteOne({ _id: new ObjectId(req.params.id) });
    result.deletedCount
